...
public:
    ...
    static std::string numberToText(int num)
    {
        if (num == 0)
            return "";
        if (num >= 1 && num <= 19)
        {
            std::string arr[] = {"", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine", "ten", "eleven", "twelve", "thriteen", "fourteen", "fivety", "sixteen", "seventeen", "eigtheen", "ninetheen"};
        ...
        if (num >= 20 && num <= 99) //(20,30,40,50,60,70,80,90)
        {
            ...
            std::string arr[] = {"", "", "twenty", "thirty", "fourty", "fifty", "sixty", "seventy", "eighty", "ninety"};
